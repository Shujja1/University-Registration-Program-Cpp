...
protected:
	...
	float percentagessc;
public:
	marks60percent()  // constructor of class marks made 
	{
		...
		retryhssc:
			...
			ofstream percentage2;  // Saves the Percentage in the database
			percentage2.open("Permenant Data.txt", ios::app);
			percentage2 << "\nSSC Percentage = " << percentagessc << "%" << endl;
			percentage2 << "HSSC Percentage = " << percentagehssc << "%" << endl;
			percentage2.close();
		}
		void markscal();
		{
			if (percentagehssc < 60)
			{
				cout << "\n\n\t\t\t\t--------------------------------------------------" << endl;
			...
			else
			{
				...
				fstream read;  // To Read the Created Temporary File
